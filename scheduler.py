...
def normalize_start_slot(slot, total_slots, inputs):
    if local_slot(slot, inputs) + total_slots > assessment_slots_per_day(inputs):
...
def lunch_target_slot(day, inputs):
    # Lunch used to aim for the middle of the window, which left everyone idle
    # in front of it and pushed the rest of the day back by that much. Aiming at
    # the earliest feasible start lets lunch absorb idle time that already
    # exists, and everything after it moves up.
    window = lunch_window_slot_range(day, inputs)
    if window is None:
        raise ValueError(f"No {LUNCH_MINUTES}-minute lunch window exists on Day {day + 1}.")

    earliest_start, _ = window
...
def lunch_start_candidates(day, inputs, earliest=None, latest=None):
    ...
    return sorted(range(min_start, max_start + 1))
...
def schedule_quality_tuple(result, inputs, end_slot_budget=None):
    ...
    # End time used to rank first, so the optimiser always accepted parking a
    # whole group for hours if it saved a few minutes at the end of the day.
    # With a budget supplied (second pass), any schedule finishing within the
    # budget is treated as equally acceptable on length, and group engagement
    # decides instead. Schedules beyond the budget are still ranked by length,
    # so the day can never run away.
    if end_slot_budget is not None and result["max_slot"] <= end_slot_budget:
        over_budget = 0
        end_rank = 0
    else:
        over_budget = 1
        end_rank = result["max_slot"]
...
def find_fastest_schedule(inputs):
        ...
        for other_rots in other_rotation_combos:
            ...
            for priority_order in priority_orders:
                for priority_mode in priority_modes:
                    ...
                    # The final budget can only shrink as shorter days are found,
                    # so anything already past it can never win and does not need
                    # scoring. The shortest candidate itself always survives.
                    if result["max_slot"] > shortest_end + tolerance_slots:
                        continue
    ...
    # Pass 2: allow the day to run up to WAIT_TOLERANCE_MINUTES past the shortest
    # option, and inside that budget prefer the schedule that keeps every group
    # engaged rather than the one that finishes a few minutes earlier.
    budget = shortest_end + tolerance_slots

    best_choice = None

    for max_slot, metrics, perm_indices, rotations, priority_order, priority_mode in candidates:
        if max_slot <= budget:
            quality = (0, 0) + metrics
        else:
            quality = (1, max_slot) + metrics
    ...
    perm_indices, rotations, priority_order, priority_mode = best_choice
    best_inputs = {
        **inputs,
    ...
    best_result = build_schedule(best_inputs)
    validate_schedule(best_result, best_inputs)
