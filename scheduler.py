...
def schedule_quality_tuple(result, inputs, end_slot_budget=None):
    ...
    weighted_delay, worst_delay = group_start_delay_score(result, inputs)
    ...
    # budget is treated as equally acceptable on length, and group fairness
...
def find_fastest_schedule(inputs):
    ...
    # Pass 1: build every valid candidate and note the shortest achievable day.
    candidates = []
    shortest_end = None
        ...
        for other_rots in other_rotation_combos:
            ...
            for priority_order in priority_orders:
                for priority_mode in priority_modes:
                    ...
                    candidates.append((perm_inputs, result))
    ...
    budget = shortest_end + (round_up_to_slots(WAIT_TOLERANCE_MINUTES) or 0)

    for perm_inputs, result in candidates:
        quality = schedule_quality_tuple(result, perm_inputs, budget)
