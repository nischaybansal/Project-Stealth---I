...
def find_fastest_schedule(inputs):
    ...
    if widened_total <= WIDENED_SEARCH_LIMIT:
        other_rotation_combos = [list(combo) for combo in product(range(n), repeat=group_count)]
    else:
        other_rotation_combos = [[0] + list(combo)
    ...
    total = (math.factorial(n) * len(other_rotation_combos) * len(priority_orders)
