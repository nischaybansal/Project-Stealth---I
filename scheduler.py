...
def cutoff_slots(tool):
    # Slots that must land before the overrun cutoff. The trailing scoring
    # phase is excluded: participants are finished by then, and the assessors
    # are heading into integration anyway, so scoring may overlap it.
    return tool_total_slots(tool) - tool["scoring_slots"]
