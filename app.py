...
def estimate_permutations(candidate_count, assessor_count, tool_count):
    ...
    # falls back to holding the first group fixed when it would not.
    widened = tool_orders * (tool_count ** group_count) * priority_orders * priority_modes
    if widened <= WIDENED_SEARCH_LIMIT:
        return widened
    return tool_orders * (tool_count ** max(group_count - 1, 0)) * priority_orders * priority_modes
...
# =======================================================
    ...
    try:
        ...
        inputs = {
            ...
            "context_slots": minutes_to_slots(cfg["context_minutes"]),
            "context_minutes": minutes_to_slots(cfg["context_minutes"]) * SLOT_MINUTES,
            "tools": tools,
        ...
        st.markdown(
            ...
            f' Tools <b>{tool_count}</b> ({", ".join(t["name"] for t in tools)})</div>',
            unsafe_allow_html=True,
        )
