...
try:
    import solver
except ImportError:  # the app still works on the legacy engine
    solver = None
...
MAX_SCORING_MINUTES = 0  # the overrun buffer already covers scoring
...
# =======================================================
    ...
    try:
        ...
        inputs = {
            ...
            # The rendered day leaves room for the overrun grace, a trailing scoring
            # phase, and the integration that follows, so a long day is not split
            # into two just because its last minutes run past End time.
            # Round the rendered day UP: with a coarse slot size the last partial
            # slot was dropped, so a schedule finishing inside it was reported as
            # a phantom Day 2 with nothing on it.
            "slots_per_day": -(-(end_minute + OVERRUN_GRACE_MINUTES + MAX_SCORING_MINUTES
                                 + integration_minutes - start_minute) // slot_minutes),
        ...
        # A schedule the user is looking at is remembered so "another option"
        # can rule it out and return a genuinely different arrangement.
        st.session_state.setdefault("shown_schedules", [])
        st.session_state.setdefault("option_number", 0)

        def build_preview(alternative):
            """Solve and stash a schedule for display. Returns an error string."""
            excluded = st.session_state["shown_schedules"] if alternative else []

            if alternative and solver is None:
                return "Alternative schedules need the solver, which is not available here."

            try:
                if alternative:
                    best_inputs, result = solver.solve_schedule(inputs, excluded=excluded)
                else:
                    st.session_state["shown_schedules"] = []
                    st.session_state["option_number"] = 0
                    best_inputs, result = find_fastest_schedule(inputs)
            except ValueError as error:
                return str(error)

            key = result.get("solution_key")
            if key is not None:
                st.session_state["shown_schedules"].append(key)
            st.session_state["option_number"] += 1
            st.session_state["preview"] = (best_inputs, result)
            return None
        ...
        with col1:
            if st.button("👁️ Preview schedule", use_container_width=True):
                with st.status("Working out the best schedule…", expanded=False):
                    problem = build_preview(alternative=False)
                if problem:
                    st.error(problem)

        with col2:
            if st.button("🔀 Another option", use_container_width=True,
                         help="A different arrangement of the same day."):
                with st.status("Looking for a different arrangement…", expanded=False):
                    problem = build_preview(alternative=True)
                if problem:
                    st.info(problem)

        if st.session_state.get("preview"):
            best_inputs, result = st.session_state["preview"]
            ...
            per_day = best_inputs["slots_per_day"]
            finish = clock(best_inputs["start_minute"] + ((last + 1) % per_day) * slot_minutes)
            spread = group_start_spread(result, best_inputs) * slot_minutes

            s1, s2, s3 = st.columns(3)
            s1.metric("Option", st.session_state["option_number"])
            s2.metric("Groups start within", f"{spread} min")
            s3.metric("Assessments end", finish)
            ...
            st.caption("Use 'Another option' to see a different arrangement, then "
                       "generate the Excel when one looks right.")

        with col3:
            if st.button("📥 Generate Excel", use_container_width=True):
                with st.status("Building the workbook…", expanded=True) as status:
                    # Export exactly the option on screen; only solve afresh if
                    # nothing has been previewed yet.
                    if st.session_state.get("preview"):
                        best_inputs, result = st.session_state["preview"]
                        st.write("Using the option shown above.")
                    else:
                        best_inputs, result = find_fastest_schedule(inputs)
                    st.write("Writing sheets…")
